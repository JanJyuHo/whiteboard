#include "BGraphicsScene.h"
#include "BGraphicsPathItem.h"
#include "BGraphicsLineItem.h"
#include "BGraphicsRectItem.h"
#include "BGraphicsEllipseItem.h"
#include "BGraphicsPixmapItem.h"
#include <QDebug>
#include <QDateTime>


BGraphicsScene::BGraphicsScene(QQuickItem *parent)
    : QQuickItem(parent)
    , m_pItem(0)
    , m_sceneScale(1.0)
{
    m_tcpSocket = NULL;

    // 获取本机IP
    QString hostIp;
    QString localHostName = QHostInfo::localHostName();
    QHostInfo info = QHostInfo::fromName(localHostName);
    foreach(QHostAddress address, info.addresses()) {
        if (address.protocol() == QAbstractSocket::IPv4Protocol) {
            hostIp = address.toString();
            break;
        }
    }

    // 用正则表达式分析IP
    QRegExp regExp("(\\d+).(\\d+).(\\d+).(\\d+)");
    regExp.setMinimal(false);
    regExp.indexIn(hostIp);
    for (int i = 0; i < 4; i++ ) {
        m_message.messageHeader.srcIP[i] = (unsigned char)(regExp.cap(i + 1).toInt());
    }
    connectToServer();

    // 设置画布
    setAcceptHoverEvents(true);
    setAcceptedMouseButtons(Qt::AllButtons);
    setFlag(ItemAcceptsInputMethod, true);
    setAntialiasing(true);

    setNormalMode();
}

BGraphicsScene::~BGraphicsScene()
{
    QDataStream out;
    out.setDevice(m_tcpSocket);
    out << disconn << m_message.messageHeader.srcIP[0] << m_message.messageHeader.srcIP[1]
        << m_message.messageHeader.srcIP[2] << m_message.messageHeader.srcIP[3];
    QCoreApplication::processEvents();
    m_tcpSocket->close();
}

void BGraphicsScene::setNormalMode()
{
    m_mode = Normal;
    setItemsAcceptedMouse(true);
    setItemsNormal();
}

void BGraphicsScene::setPenMode(int width, const QColor &color)
{
    m_mode = Pen;
    m_penWidth = width;
    m_penColor = color;
    setItemsAcceptedMouse(false);
    setItemsNormal();
}

void BGraphicsScene::setLightMode()
{

}

void BGraphicsScene::setShapeMode(int type, int width, const QColor &color)
{
    m_mode = (SceneMode)type;
    m_shapeWidth = width;
    m_shapeColor = color;
    setItemsAcceptedMouse(false);
    setItemsNormal();
}

void BGraphicsScene::setTextMode(int width, const QString &name, const QColor &color)
{
    m_mode = Text;
    setItemsAcceptedMouse(false);
    setItemsNormal();
}

void BGraphicsScene::setEraserMode()
{
    m_mode = Eraser;
    setItemsAcceptedMouse(false);
    setItemsNormal();
}

void BGraphicsScene::addPixmap(float x, float y, const QString &pixmap)
{
    BGraphicsPixmapItem *item = new BGraphicsPixmapItem(this);
    item->drawPixmap(x, y, pixmap);
}

void BGraphicsScene::setSceneScale(qreal scale, qreal x, qreal y)
{
    m_sceneScale = scale;
    QList<QQuickItem*> list = childItems();
    foreach (QQuickItem* item, list)
    {
        item->setX(item->x() * x);
        item->setY(item->y() * y);
        BGraphicsAbstractItem *pItem = dynamic_cast<BGraphicsAbstractItem*>(item);
        if (pItem)
        {
            pItem->setWidth(pItem->width()*x);
            pItem->setHeight(pItem->height()*y);
            pItem->setScaled();
        }
    }
}

void BGraphicsScene::setItemsAcceptedMouse(bool enabled)
{
    foreach (QQuickItem* item, childItems())
    {
        if (enabled)
            item->setAcceptedMouseButtons(Qt::AllButtons);
        else
            item->setAcceptedMouseButtons(Qt::NoButton);
    }
}

void BGraphicsScene::setItemsNormal()
{
    foreach (QQuickItem* item, childItems())
    {
        item->setFocus(false);
        BGraphicsAbstractItem *pItem = static_cast<BGraphicsAbstractItem*>(item);
        if (pItem)
            pItem->setSelected(false);
    }
}

void BGraphicsScene::connectToServer()
{
    m_tcpSocket = new QTcpSocket(this);
    m_tcpSocket->connectToHost(QHostAddress::LocalHost, 11694);
    qDebug() << QHostAddress::LocalHost;
    connect(m_tcpSocket, &QTcpSocket::readyRead, this, &BGraphicsScene::getData, Qt::UniqueConnection);
    connect(m_tcpSocket, &QTcpSocket::connected, this, &BGraphicsScene::sendRequest, Qt::UniqueConnection);
    connect(m_tcpSocket, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(error()));
    connect(m_tcpSocket, &QTcpSocket::disconnected, this, &BGraphicsScene::error, Qt::UniqueConnection);
}

void BGraphicsScene::paint()
{
    int beginX = m_message.userOpInfo.startPoint.x;
    int beginY = m_message.userOpInfo.startPoint.y;
    int endX = m_message.userOpInfo.endPoint.x;
    int endY = m_message.userOpInfo.endPoint.y;
    switch (m_message.userOpInfo.opKind) {
    case Pen: {
        BGraphicsPathItem *item = new BGraphicsPathItem(this);
        m_pItem = item;
        item->setPen(QPen(m_penColor, m_penWidth));
        item->setPenScale(m_sceneScale);
        item->drawPath(QPointF(beginX, beginY));
        break;
    }
    case Line: {
        BGraphicsLineItem *item = new BGraphicsLineItem(this);
        m_pItem = item;
        item->setPen(QPen(m_shapeColor, m_shapeWidth));
        item->setPenScale(m_sceneScale);
        item->drawLine(QPointF(beginX, beginY), QPointF(endX, endY));
        break;
    }
    case Rect: {
        BGraphicsRectItem *item = new BGraphicsRectItem(this);
        item->setPen(QPen(m_shapeColor, m_shapeWidth));
        item->setPenScale(m_sceneScale);
        m_pItem = item;
        item->drawRect(QPointF(beginX, beginY), QPointF(endX, endY));
        break;
    }
    case Ellipse:
    {
        BGraphicsEllipseItem *item = new BGraphicsEllipseItem(this);
        item->setPen(QPen(m_shapeColor, m_shapeWidth));
        item->setPenScale(m_sceneScale);
        m_pItem = item;
        item->drawEllipse(QPointF(beginX, beginY), QPointF(endX, endY));
        break;
    }
    case Text:
    {
        QVariant retVal;
        QMetaObject::invokeMethod(this, "addText", Qt::DirectConnection,
                                  Q_RETURN_ARG(QVariant, retVal),
                                  Q_ARG(QVariant, beginX),
                                  Q_ARG(QVariant, beginY),
                                  Q_ARG(QVariant, m_sceneScale));
        break;
    }
    default:
        return;
    }
}

void BGraphicsScene::getData()
{
    int mark;
    QDataStream in(m_tcpSocket);
    in >> mark >> m_message.messageHeader.desIP[0] >> m_message.messageHeader.desIP[1] >> m_message.messageHeader.desIP[2] >> m_message.messageHeader.desIP[3];
    QString userIp = QString("%1.%2.%3.%4").arg(m_message.messageHeader.desIP[0]).arg(m_message.messageHeader.desIP[1]).arg(m_message.messageHeader.desIP[2]).arg(m_message.messageHeader.desIP[3]);
    if (painted == mark) {
        in >> m_message.userOpInfo.opKind >> m_message.userOpInfo.startPoint.x >> m_message.userOpInfo.startPoint.y >> m_message.userOpInfo.endPoint.x >> m_message.userOpInfo.endPoint.y;
        paint();
    }
    else if (disconn == mark) {
        // todo:
        qDebug() << "disconnected!";
    }
}

void BGraphicsScene::sendRequest()
{
    qDebug() << "login successfully";
    QDataStream out;
    out.setDevice(m_tcpSocket);
    out << conn << m_message.messageHeader.srcIP[0] << m_message.messageHeader.srcIP[1] << m_message.messageHeader.srcIP[2] << m_message.messageHeader.srcIP[3];
    disconnect(m_tcpSocket, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(error()));
}

void BGraphicsScene::error()
{
    qDebug() << "disconnected!";
}

void BGraphicsScene::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        m_posPressed = event->pos();
        switch (m_mode)
        {
        case Normal:
        {
            setItemsNormal();
            break;
        }
        case Pen:
        {
            m_message.userOpInfo.opKind = Pen;
            BGraphicsPathItem *item = new BGraphicsPathItem(this);
            m_pItem = item;
            item->setPen(QPen(m_penColor, m_penWidth));
            item->setPenScale(m_sceneScale);
            item->drawPath(event->pos());
            break;
        }
        case Line:
        {
            m_message.userOpInfo.opKind = Line;
            BGraphicsLineItem *item = new BGraphicsLineItem(this);
            m_pItem = item;
            item->setPen(QPen(m_shapeColor, m_shapeWidth));
            item->setPenScale(m_sceneScale);
            item->drawLine(event->pos(), event->pos());
            break;
        }
        case Rect:
        {
            m_message.userOpInfo.opKind = Rect;
            BGraphicsRectItem *item = new BGraphicsRectItem(this);
            item->setPen(QPen(m_shapeColor, m_shapeWidth));
            item->setPenScale(m_sceneScale);
            m_pItem = item;
            item->drawRect(event->pos(), event->pos());
            break;
        }
        case Ellipse:
        {
            m_message.userOpInfo.opKind = Ellipse;
            BGraphicsEllipseItem *item = new BGraphicsEllipseItem(this);
            item->setPen(QPen(m_shapeColor, m_shapeWidth));
            item->setPenScale(m_sceneScale);
            m_pItem = item;
            item->drawEllipse(event->pos(), event->pos());
            break;
        }
        case Text:
        {
            m_message.userOpInfo.opKind = Text;
            QVariant retVal;
            QMetaObject::invokeMethod(this, "addText", Qt::DirectConnection,
                                      Q_RETURN_ARG(QVariant, retVal),
                                      Q_ARG(QVariant, event->pos().x()),
                                      Q_ARG(QVariant, event->pos().y()),
                                      Q_ARG(QVariant, m_sceneScale));
            break;
        }
        default:
            return;
        }
        m_message.userOpInfo.startPoint.x = event->x();
        m_message.userOpInfo.startPoint.y = event->y();
    }
}

void BGraphicsScene::mouseReleaseEvent(QMouseEvent *event)
{
    m_message.userOpInfo.endPoint.x = event->x();
    m_message.userOpInfo.endPoint.y = event->y();
    if (m_message.userOpInfo.endPoint.x == m_message.userOpInfo.startPoint.x && m_message.userOpInfo.endPoint.y == m_message.userOpInfo.startPoint.y)
        return;
    QDataStream out;
    out.setDevice(m_tcpSocket);
    out << painted << m_message.messageHeader.srcIP[0] << m_message.messageHeader.srcIP[1]
        << m_message.messageHeader.srcIP[2] << m_message.messageHeader.srcIP[3]
        << m_message.userOpInfo.opKind << m_message.userOpInfo.startPoint.x
        << m_message.userOpInfo.startPoint.y << m_message.userOpInfo.endPoint.x
        << m_message.userOpInfo.endPoint.y;
}

void BGraphicsScene::mouseMoveEvent(QMouseEvent *event)
{
    /*if (!contains(event->pos()))
        return;*/
    switch (m_mode)
    {
    case Pen:
    {
        /*static qint64 msce = 0;
        if (QDateTime::currentMSecsSinceEpoch() - msce < 20)
            return;
        msce = QDateTime::currentMSecsSinceEpoch();*/
        if (m_pItem.isNull())
            return;
        BGraphicsPathItem *pathItem = dynamic_cast<BGraphicsPathItem*>(m_pItem.data());
        if (pathItem)
            pathItem->drawPath(event->pos());
        break;
    }
    case Line:
    {
        if (m_pItem.isNull())
            return;
        BGraphicsLineItem *lineItem = dynamic_cast<BGraphicsLineItem*>(m_pItem.data());
        if (lineItem)
            lineItem->drawLine(m_posPressed, event->pos());
        break;
    }
    case Rect:
    {
        if (m_pItem.isNull())
            return;
        BGraphicsRectItem *rectItem = dynamic_cast<BGraphicsRectItem*>(m_pItem.data());
        if (rectItem)
            rectItem->drawRect(m_posPressed, event->pos());
        break;
    }
    case Ellipse:
    {
        if (m_pItem.isNull())
            return;
        BGraphicsEllipseItem *ellipseItem = dynamic_cast<BGraphicsEllipseItem*>(m_pItem.data());
        if (ellipseItem)
            ellipseItem->drawEllipse(m_posPressed, event->pos());
        break;
    }
    case Eraser:
    {
        onDealEraser(event->pos());
        break;
    }
    default:
        break;
    }
    m_message.userOpInfo.endPoint.x = event->x();
    m_message.userOpInfo.endPoint.y = event->y();
}

void BGraphicsScene::onDealEraser(const QPointF &pos)
{
    QList<QQuickItem*> list = childItems();
    foreach (QQuickItem* item, list)
    {
        QRectF tR = item->mapRectToItem(this, item->boundingRect());
        if (tR.contains(pos))
        {
            item->setParent(nullptr);
            item->setParentItem(nullptr);
            item->deleteLater();
        }
    }
}

void BGraphicsScene::hoverMoveEvent(QHoverEvent *event)
{
    QQuickItem::hoverMoveEvent(event);
}

void BGraphicsScene::wheelEvent(QWheelEvent *event)
{
    QPointF numPixels = event->pixelDelta();
    QPointF numDegrees = event->angleDelta() / 8;

    if (!numPixels.isNull()) {
        //scrollWithPixels(numPixels);
    } else if (!numDegrees.isNull()) {
        QPointF numSteps = numDegrees / 15;
        numSteps = numSteps*30 + position();
        if (numSteps.y() > 0)
            numSteps.setY(0);
        setPosition(numSteps);
    }
}

